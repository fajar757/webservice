<?php 
    ...
    $root = $doc->createElement('akademik');                      //Buat element baru <akademik>
    $root = $doc->appendChild($root);                        
  
    $query=mysql_query("select nim,nama,alamat,progdi from mahasiswa");         
    while ( $get_data = mysql_fetch_object($query)) 
    {
        $item= $doc->createElement('mahasiswa');                 //buat element baru <mahasiswa>
        $item = $root->appendChild($item);                  //masukkan sebagai anak dari element <akademik>
